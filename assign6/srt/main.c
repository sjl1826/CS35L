...
void* outputFunction(void* thready) {
    Vec3 camera_pos;
    ...
    /* for every pixel */
    for( int px= *(int*)thready; px<width; px+= nthreads )
    {
...
main( int argc, char **argv )
{
    ...
    for(i = 0; i<nthreads; i++) {
      threadargs[i] = i;
      int ret = pthread_create(&threads[i], NULL, outputFunction, &threadargs[i]);
      if(ret) {
    ...
    for(t = 0; t<nthreads; t++) {
      int ret1 = pthread_join(threads[t], NULL);
      if(ret1) {
	...
	printf( "%.0f %.0f %.0f\n",
		scaled_color[w][h][0], scaled_color[w][h][1], scaled_color[w][h][2] );
      }
      printf("\n");
    }
